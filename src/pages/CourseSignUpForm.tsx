...
//         toast.success("Account created! Redirecting to course...", { id: "signup" });
...
//       }
//     } catch (error: any) {
//       toast.error(error?.response?.data?.message || "Submission Failed", { id: "signup" });
//     } finally {
...
const CourseSignUpForm: React.FC<CourseSignUpFormProps> = React.memo(({
    ...
    const handleSignup = useCallback(async (data: EnrollmentFormFields) => {
        ...
        try {
            ...
            const response: any = await axiosInstance.post("auth/personal-info", formData);
            const { token, user, existing } = response.data;
            ...
            if (token && user) {
                ...
                // Redirect immediately after setting token for the fastest transition
                navigate(`/course-detail/${actualCourseId}?autoEnroll=true`);
            }
        } catch (error: any) {
